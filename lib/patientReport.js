...
export async function buildPatientReport({ user, intake, latestSession, diagnosisSessions = [] }) {
    ...
    const provider = chatProviderConfig({ fallbackModel: REPORT_MODEL });
    if (!provider) {
        return {
            report: fallback,
            text: reportToText(fallback),
            generatedBy: "fallback",
            warning: "AI provider is not configured. Generated report from saved diagnosis data."
        };
    }
    ...
    try {
        const result = await fetchChatCompletion({
            ...
            temperature: 0.15,
            maxTokens: 1100,
            fallbackMessage: "Report generation failed. Generated report from saved diagnosis data.",
            messages: [
