...
function isOpenCodeUrl(url) {
    return String(url || "").startsWith("https://opencode.ai/");
}
...
export function openAiCompatibleChatPayload({
    model,
    messages,
    maxTokens,
...
export async function fetchChatCompletion({
    ...
    const opencodeRequest = isOpenCodeUrl(url);
    return fetchProviderJson({
        ...
        body: JSON.stringify(openAiCompatibleChatPayload({
            ...payload,
            maxTokenParam: opencodeRequest ? "max_completion_tokens" : "max_tokens",
            thinking: opencodeRequest ? { type: "disabled" } : undefined
        }))
    });
}
