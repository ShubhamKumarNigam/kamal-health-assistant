...
async function sendDueReminder(reminder) {
    ...
    if (result.ok) {
        ...
        if (nextDate) {
            await rescheduleMedicationReminder(reminder.id, nextDate);
            scheduleMedicationReminder({ ...reminder, scheduled_at: nextDate.toISOString() });
            return;
        }
        await markMedicationReminderSent(reminder.id);
        return;
    }
    await markMedicationReminderFailed(reminder.id, result.message);
}

export async function processDueMedicationReminders() {
    const due = await listDueMedicationReminders();
...
export function scheduleMedicationReminder(reminder) {
    ...
    const timer = setTimeout(() => {
        sendDueReminder(reminder).catch((error) => {
            markMedicationReminderFailed(reminder.id, error instanceof Error ? error.message : "Email failed").catch((failure) => {
                console.error("Medication reminder failure update failed", failure);
            });
...
export async function refreshMedicationReminderSchedules() {
    for (const reminder of await listScheduledMedicationReminders()) {
