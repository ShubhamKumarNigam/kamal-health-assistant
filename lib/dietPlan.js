...
export async function buildDietPlan({ user, intake, latestSession, report }) {
    ...
    const provider = chatProviderConfig({ fallbackModel: DIET_MODEL });
    if (!provider) {
        return {
            plan: fallback,
            generatedBy: "fallback",
            warning: "AI provider is not configured. Generated a stable fallback diet plan from saved diagnosis data."
        };
    }
    ...
    try {
        const result = await fetchChatCompletion({
            ...
            topP: 0.2,
            maxTokens: 800,
            fallbackMessage: "Diet model failed. Generated a stable fallback diet plan from saved diagnosis data.",
            messages: [
